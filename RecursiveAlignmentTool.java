...
public class RecursiveAlignmentTool {
    ...
    private void checkWordLevelAlignment() {
        ...
        System.out.println("Check successful " + refTokens.length + "\t" + candTokens.length + "\t" + alignment.size());
    }
    ...
    private void charAlignSubsequences() {
        ...
        for (int i = 0; i <= alignment.size(); i++) {
            ...
            // perform a check to see if the data is too big to align (efficiently) via
            // dynamic programming. If they are, just put them in the alignment. This also
            // prevents the refAccuSize and/or candAccuSize indexes from going
            // out of bounds. 
            // Note we're including the lengths of the current words we're processing
            // to ensure we won't go beyond the array bounds once those words get
            // added to the accumulate arrays. For example the candAccuSize could be zero,
            // and the refAccuSize could be (MAX_DYANMIC_TABLE_SIZE - 1), without the
            // word length, the array would blow up later on.
            // We're also using MAX in the calc to ensure we don't mulitply by zero, which would cause this "if"
            // to return false but array access below would be out of bounds if either size
            // is >= MAX_DYNAMIC_TABLE_SIZE and the other is zero.

            if ((long) Math.max(refAccuSize + ((ref == null) ? 0 : ref.length()), 1)
                    * (long) Math.max(candAccuSize + ((cand == null) ? 0 : cand.length()), 1) >= MAX_DYNAMIC_TABLE_SIZE) {
                ...
                refAccuSize = 0;
                candAccuSize = 0;
                // fall through and let the existing logic handle the can/ref words
            } // end too big
                ...
                if (cand.equals(ref)) { // matching words?
                    ...
                    if (refAccuSize == 0) {
                        ...
                        if (candAccuSize == 0) {
                            ...
                            alignment2.addAll(sq);// add it to the resulting sequence     
                        }
                    }
                    ...
                    refAccuSize = 0;
                    candAccuSize = 0;
                    //   System.out.println("                    " + i + ": resetting accumulators");
                } else { // otherwise we have BOTH a candidate and reference word but they do not match
            ...
            } else { // either candidate or refrence word is null
                if (cand == null) {
            ...
            } // end if either candidate or ref word is null
