    ...
    private void charAlignSubsequences() {
        ...
        for ( int i = 0; i <= alignment.size(); i++ ) {
            ...
            if ( cand != null && ref != null ) {
                if (cand.equals(ref)){ // matching words?
                ...
                }else{ // otherwise add the characters of both words into the character accumulators 
                                           
                    for ( int ll = 0; ll <= ref.length(); ll++){
                        if ( ref.length() == ll){
                            ch = " ";
                        }else{
                            ch = ref.substring( ll, ll+1 );
                        }                        
                        if ( refAccu.length > refAccuSize ) {
                            refAccu[refAccuSize++] = ch;
                        }                       
                    }
                    ...
                    for ( int ll = 0; ll <= cand.length(); ll++){
                        if ( cand.length() == ll){
                            ...
                            ch = cand.substring( ll, ll+1 );
                        }  
                        if ( candAccu.length > candAccuSize ) {
                            candAccu[candAccuSize++] = ch;
                        }
                    }                  
                }
            }else{
                if ( cand == null ){ // add the characters of the word in the reference book to the character accumulator
                    for ( int ll = 0; ll <= ref.length(); ll++){
                        if ( ref.length() == ll){
                            ch = " ";
                        }else{
                ...
                }else{ // that means cur.m_reference is null, add the characters of the word in the candidate book to the character accumulator
                     for ( int ll = 0; ll <= cand.length(); ll++){
                        if ( cand.length() == ll){
                            ch = " ";
                        }else{
